...
namespace PointCloudConverterForDotnetCLI
{
    public class PCC
    {
        ...
        private string appname = $"PointCloud Converter For Dotnet CLI - 1.0";

        private List<LasHeader> lasHeaders = new List<LasHeader>();
        private string lastStatusMessage = "";
        private int errorCounter = 0;

        private CancellationTokenSource _cancellationTokenSource = new CancellationTokenSource();
        private string externalFileFormats = "";

        private Dictionary<string, Type> externalWriters = new Dictionary<string, Type>();
        private ILogger Log;

        private int progressPoint = 0;
        private int progressTotalPoints = 0;
        private int progressFile = 0;
        private int progressTotalFiles = 0;

        public bool offset = true;
        public bool rgb = true;
        public bool intensity = true;
        public int gridsize = 5;
        public int minpoints = 1000;
        public double scale = 0.1;
        public bool swap = true;
        public bool pcak = true;
        ...
        public string PCC_Start(string LASFileName, string PCRootFileName)
        {
            string strRet = string.Empty;
            FileInfo fi = new FileInfo(LASFileName);
            FileInfo pcInfo = new FileInfo(PCRootFileName);
            pcInfo.Directory?.Create();

            if (fi.Exists)
            {
                string [] strarg = [$"-input=\"{fi.FullName}\"", 
                                     "-importformat=LAS", 
                                     "-exportformat=PCROOT",
                                    $"-output=\"{pcInfo.FullName}\"",
                                     $"-offset={BoolToStr(offset)}", 
                                     $"-rgb={BoolToStr(rgb)}", 
                                     $"-intensity={BoolToStr(intensity)}", 
                                     $"-gridsize={gridsize}", 
                                     $"-minpoints={minpoints}", 
                                     $"-scale={scale}", 
                                     $"-swap={BoolToStr(swap)}", 
                                     $"-pack={BoolToStr(pcak)}", 
                                     $"-randomize={BoolToStr(randomize)}", 
                                     $"-seed={seed}"];
                RunAsync( strarg ).Wait();
                strRet = pcInfo.FullName;
            }
            return strRet;
        }
        ...
        public async Task RunAsync(string [] args)
        {
            ...
            if (args.Length > 1)
            {
                ...
                if (importSettings.useJSONLog)
                {
                    Log.Write("{\"event\": \"" + LogEvent.End + "\", \"elapsed\": \"" + elapsedString + "\",\"version\":\" 1.0 ,\"errors\":" + errorCounter + "}", LogEvent.End);
                }
